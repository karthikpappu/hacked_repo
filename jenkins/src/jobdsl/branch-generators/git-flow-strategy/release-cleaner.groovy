	...
	steps {
			...
			downstreamParameterized {		
				trigger ( 'delete-git-branch', 'ALWAYS', false,
					...
					predefinedProps ( [ "GIT_REPO" : "\${SOURCE_REPOSITORY}",
										"GIT_BRANCH_TO_DELETE" : "${gitBranchType}/\${RELEASE_BRANCH}",
										"PREVIEW" : "\${PREVIEW}" ] )
				}
			}
		}
		...
		downstreamParameterized {		
			trigger ( 'delete-jobs-by-regex', 'ALWAYS', false,
				...
				predefinedProps ( [ "regularExpression" : "\${PRODUCT}-\${COMPONENT}-${gitBranchTypeAbbr}-\${RELEASE_BRANCH}-.*",
									"preview" : "\${PREVIEW}" ] )									
			}
		}
