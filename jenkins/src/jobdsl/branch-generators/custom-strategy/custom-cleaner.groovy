job {
	...
	name ( "delete-${PRODUCT}-${COMPONENT}-custom-pipeline" )
	...
	steps {
			...
			downstreamParameterized {		
				trigger ( 'delete-git-branch', 'ALWAYS', false,
					...
					predefinedProps ( [ "GIT_REPO" : "\${SOURCE_REPOSITORY}",
										"GIT_BRANCH_TO_DELETE" : "\${GIT_TARGET_BRANCH}",
										"PREVIEW" : "\${PREVIEW}" ] )
		...
		downstreamParameterized {		
			trigger ( 'delete-jobs-by-regex', 'ALWAYS', false,
				...
				predefinedProps ( [ "regularExpression" : "\${PRODUCT}-\${COMPONENT}-\${GIT_TARGET_BRANCH}-.*",
									"preview" : "\${PREVIEW}" ] )									
			}
		}
